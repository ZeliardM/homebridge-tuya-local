...
class RGBTWLightAccessory extends BaseAccessory {
  ...
  _registerCharacteristics(dps: DPSState): void {
    ...
    const typeBDatapoints =
      this.device.context.version === '3.5' || dps.hasOwnProperty('20') || dps.hasOwnProperty('21')
    const defaultDps = typeBDatapoints
      ? { power: '20', mode: '21', brightness: '22', colorTemperature: '23', color: '24' }
      : { power: '1', mode: '2', brightness: '3', colorTemperature: '4', color: '5' }

    this.dpPower = this._getCustomDP(this.device.context.dpPower) || defaultDps.power
    this.dpMode = this._getCustomDP(this.device.context.dpMode) || defaultDps.mode
    this.dpBrightness = this._getCustomDP(this.device.context.dpBrightness) || defaultDps.brightness
    this.dpColorTemperature = this._getCustomDP(this.device.context.dpColorTemperature) || defaultDps.colorTemperature
    this.dpColor = this._getCustomDP(this.device.context.dpColor) || defaultDps.color

    if (typeBDatapoints) {
      if (!this.device.context.minBrightness) this.device.context.minBrightness = 10
