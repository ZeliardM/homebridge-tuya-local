...
class BaseAccessory {
  ...
  constructor(...props: any[]) {
    ...
    this.device.once('connect', () => {
      ...
      const timeout = Number(this.device.context.initialDpsTimeout) || 15000
      readyTimer = setTimeout(() => {
        if (ready) return

        this.log.warn(
          '%s (%s:%s) connected but has not reported state after %s seconds; HomeKit services are waiting for the first state report.',
          this.device.context.name,
          this.device.context.type,
          this.device.context.version,
    ...
    this.device.once('change', () => {
      ...
      this.log(
        `Ready to handle ${this.device.context.name} (${this.device.context.type}:${this.device.context.version}).`,
      )

      this._registerCharacteristics(this.device.state)
    })

    this.device.on('disconnect', (err?: Error) => {
      this._markServicesNotResponding(err instanceof Error ? err : new Error('Not connected'))
    })
  ...
  _markServicesNotResponding(error: Error): void {
    const { Service, Characteristic } = this.hap
    const accessoryInfoUUID = (Service.AccessoryInformation as any)?.UUID
    const nameUUID = (Characteristic.Name as any)?.UUID

    ;((this.accessory as any).services || []).forEach((service: any) => {
      if (!service || service.UUID === accessoryInfoUUID) return
      ;(service.characteristics || []).forEach((characteristic: any) => {
        if (!characteristic || characteristic.UUID === nameUUID || typeof characteristic.updateValue !== 'function')
          return
  ...
  _getServiceById(serviceType: any, id: string): HAPService | undefined {
    const accessory = this.accessory as any
    if (typeof accessory.getServiceById === 'function') {
      return accessory.getServiceById(serviceType, id)
    }

    if (typeof accessory.getServiceByUUIDAndSubType === 'function') {
      return accessory.getServiceByUUIDAndSubType(serviceType, id)
    }
  }
  ...
  _detectColorFunction(value: string | undefined): void {
    ...
    if (!this.colorFunction) {
      ...
      this.log.debug(`Color format for ${this.device.context.name} (${this.device.context.version}) is undetectable.`)
