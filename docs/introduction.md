...
`homebridge-tuya-local` communicates directly with Tuya devices on your local network using their Device ID and Local Key — no cloud dependency required.

## Features

- **Local LAN control** — no Tuya cloud needed after initial key extraction
- **Wide device support** — 24 device type handlers covering most Tuya-based products
...
| `UniversalRemoteDoor`   | Door               | Tuya RF universal remote door with integrated DIRIGERA contact state |
...
- **Node.js** 22.12.0 or newer on Node 22, or Node 24.x
