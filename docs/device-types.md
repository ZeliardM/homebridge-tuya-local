...
| Universal Remote Door                |  `UniversalRemoteDoor`  | Tuya RF universal remote door with integrated DIRIGERA contact state ([details](#universal-remote-doors)) |
...
If your garage door or garage door opener does more than just open and close (e.g., reports its position or detects obstacles), please [create an issue](https://github.com/nubisco/homebridge-tuya-local/issues/new) and paste your signature with any information you can provide.
:::
...
### Universal Remote Doors

`UniversalRemoteDoor` is for a door motor controlled through a Tuya IR/RF universal remote, with real door state from a DIRIGERA contact sensor. It exposes a HomeKit door tile, the DIRIGERA sensor as the accessory's integrated contact sensor, and a security system service where Home = manual mode and Away = automatic mode.
...
```json5
{
  ...
  /* Learned RF code that toggles the door open/closed while in automatic mode */
  doorToggleCode: 'LEARNED_DOOR_TOGGLE_RF_CODE',

  /* Learned RF codes for the door motor's mode buttons */
  automaticModeCode: 'LEARNED_AUTOMATIC_MODE_RF_CODE',
  manualModeCode: 'LEARNED_MANUAL_MODE_RF_CODE',

  /* DIRIGERA contact sensor state */
  dirigeraGatewayIP: '192.168.1.50',
  dirigeraAccessToken: 'DIRIGERA_ACCESS_TOKEN',
  dirigeraDeviceId: 'IKEA_CONTACT_SENSOR_ID',
}
```

To learn RF codes without running a separate helper script, set `rfSetupMode` in Homebridge Config UI, restart Homebridge, press the physical remote button, then copy the learned code from the Homebridge log into the matching field and set `rfSetupMode` back to `off`.

Useful setup modes:

```json5
{
  rfSetupMode: 'learnDoorToggle',
  rfLearnTimeoutSeconds: 30,
}
```

`rfSetupMode` can learn `learnDoorToggle`, `learnAutomaticMode`, or `learnManualMode`. It can also test configured codes with `testDoorToggle`, `testAutomaticMode`, or `testManualMode`.
