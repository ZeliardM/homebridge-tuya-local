...
```bash
git clone https://github.com/nubisco/homebridge-tuya-local.git homebridge-tuya-local
cd homebridge-tuya-local
npm install
```
...
Conventional Commits are preferred to keep history readable:
...
Documentation is kept as Markdown only in this private fork. The old VitePress build and GitHub Pages deployment have been removed.
...
- **Plugin version**: Check in Homebridge UI or `npm list homebridge-tuya-local`
- **Device information**:
...
This fork is private/local-use only. semantic-release and npm publishing have been removed.

## Questions?

Open a [discussion](https://github.com/nubisco/homebridge-tuya-local/discussions) for general questions or feature requests.
